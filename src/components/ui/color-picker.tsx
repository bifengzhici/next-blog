...
export function ColorPicker({
    ...
    const handleColorChange = (newColor: string) => {
        ...
        setHsl([h, s, l])
        onChange(`hsl(${h}, ${Math.round(s)}%, ${Math.round(l)}%)`)
    }
    ...
    const handleColorInputChange = (
        ...
        if (
            ...
            /^hsl\(\s*(\d{1,3})(\.\d+)?\s*,\s*(\d{1,3})%\s*,\s*(\d{1,3})%\s*\)$/i.test(newColor)
        ) {
    ...
    return (
        <Popover open={isOpen} onOpenChange={setIsOpen}>
            <PopoverTrigger asChild>
                <VscSymbolColor className="text-2xl sm:text-3xl text-theme-60 cursor-pointer" />
            </PopoverTrigger>
